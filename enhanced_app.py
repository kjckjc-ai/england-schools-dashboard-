...
@st.cache_data
def search_schools(name="", trust_name="", la="", school_types=None, phase="", postcode="", page=1, per_page=20):
    conn = get_connection()
...
def main():
    ...
    trust_filter = st.sidebar.text_input("Trust Name")
    ...
    type_filter = st.sidebar.multiselect("Establishment Type(s)", type_options)
    ...
    if not schools.empty:
        ...
        display_df = schools[['URN', 'EstablishmentName', 'LA (name)', 'TypeOfEstablishment (name)', 'PhaseOfEducation (name)', 'Trusts (name)', 'Postcode']]
        display_df.columns = ['URN', 'School Name', 'Local Authority', 'Type', 'Phase', 'Trust', 'Postcode']
    ...
    if hasattr(st.session_state, 'view_trust'):
        ...
        if not trust_schools.empty:
            # Create a DataFrame with only the columns we want to display
            trust_display_df = trust_schools[['URN', 'EstablishmentName', 'LA (name)', 'TypeOfEstablishment (name)', 'PhaseOfEducation (name)', 'Postcode']]
            trust_display_df.columns = ['URN', 'School Name', 'Local Authority', 'Type', 'Phase', 'Postcode']
